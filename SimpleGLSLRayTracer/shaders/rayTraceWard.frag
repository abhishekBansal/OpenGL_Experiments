...
{
	vec3 normal,lightDir,halfVector,viewV;
	vec3 n,ldir;
	float NdotL,NdotHV;
	...
	if (NdotL > 0.0) {
		...
		diffuse1 = mat_diffuse* diffuse *shadowFactor ;
		color += diffuse1 * NdotL;
		
		//calculating ward anisotropic highlights
        ...
        float dotVN = dot(viewV, n);
        float dotHTAlphaX = dot(halfVector, tangent) / alphaX;
        float dotHBAlphaY = dot(halfVector, binormal) / alphaY;
		
		color += mat_specular * specular * shadowFactor * sqrt(max(0.0, NdotL / dotVN)) 
