...
//	halfVector = normalize(lightDir + viewV);
	...
	if (NdotL > 0.0) {
		...
		RdotV = max(dot(refVector,viewV),0.0);
		color += mat_specular * specular * pow(RdotV,shininess) * shadowFactor;
	}
	return color.rgb;
}
