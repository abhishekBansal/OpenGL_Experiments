...
char * abShader::abTextFileRead(char *fn) {
	...
	if (fn != NULL)
	{
		...
		else 
		{
			...
			if (count > 0) {
				std::cout<<"\nNow trying to read shader file named : "<<fn<<"\n";
...
void abShader::abSetUniformVariable(char *name, int value)
{  
	glUseProgram(programID);
	GLint parameterLocation = glGetUniformLocation(programID, name);
...
void abShader::abSetUniformVariable(char *name, float *value)
{
	glUseProgram(programID);
	GLint parameterLocation = glGetUniformLocation(programID, name);
	glUniform3f(parameterLocation,value[0],value[1],value[2]);
	glUseProgram(0);
}
void abShader::abStopProgram()
{
	glUseProgram(0);
}